{...}: {
  flake.homeModules.cli-editor-markdown-oxide = {
    config,
    pkgs,
    ...
  }: let
    tomlFormat = pkgs.formats.toml {};
    genConfig = tomlFormat.generate "moxide-config";
  in {
    home.packages = [
      pkgs.markdown-oxide
    ];
    home.file."${config.xdg.configHome}/moxide/settings.toml".source = genConfig {
      dailynote = "%d-%m-%Y";
